    ...
    transformIndexHtml(html) {
      return html.replace('<!-- PROJECT_CARDS -->', projects.map((p, i) => {
        ...
        return `<article class="project-card" data-group="${p.group}" aria-labelledby="project-${p.slug}">
          <figure class="project-visual"><img src="/Portfolio/assets/projects/${p.image}" alt="${escape(p.alt)}" width="900" height="540" loading="lazy" decoding="async"><span class="project-number mono">${String(i+1).padStart(2,'0')}</span><figcaption>${escape(source)}</figcaption></figure>
